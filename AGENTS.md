...
| `~/.config/omarchy/plugins/najm.downloads/` | Omarchy bar-widget: a second client of the same agent socket — progress/queue monitor with cancel/pause/resume (see "Widget (`najm.downloads`)") |
...
```bash
omarchy plugin validate .                      # in ~/.config/omarchy/plugins/najm.downloads/
qmllint Panel.qml Client.js Formats.js         # syntax/type check the QML + JS
node /tmp/opencode/test-formats.mjs            # Formats.js unit tests (buildSelection/labels)
node /tmp/opencode/test-probe-fsm.mjs          # probe-reply FSM mirror of onProbeReply
```
...
(`theme.js` → `background-4.js` → native port → shim → agent).

## Theme pipeline (live Omarchy colors)

`background-4.js` handles `getTheme` with a ~4s cache: it forwards the request
...
- **Chromium caches MV3 service workers for `--load-extension` extensions and
  ...
  (`background-4.js`, same trick as Omarchy's `copy-url`): bump the filename on
...
A bar-widget at `~/.config/omarchy/plugins/najm.downloads/` that is a **second
client of the same agent socket** (`$XDG_RUNTIME_DIR/najm-ytdlp/agent.sock`,
...
of `background-4.js` — it talks JSON-lines to the agent directly, with no
4-byte framing (that is shim-only).

| File | Role |
|---|---|
| `manifest.json` | bar-widget plugin manifest (id `najm.downloads`) |
| `Panel.qml` | shared-queue monitor + all logic: progress view, cancel/pause/resume the active job, queue reorder/remove, connection state (only file with QML; hot-reloaded by the shell watcher, reliable pickup via `omarchy restart shell`) |
